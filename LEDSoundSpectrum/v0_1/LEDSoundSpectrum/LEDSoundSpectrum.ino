...
/**
 * @fn Task_ChangeColors
 * @brief listens for user button input, which is used to alternate between color 
 * scale schemes
*/
void Task_ChangeColors(void *pvParameters);
...
void Task_Setup(void *pvParameters) {
  ...
  Q_fft_moving_avg = xQueueCreate(1, sizeof(float*));
  ...
  // FFT weighted moving average values array, weight setting variable
  float* fft_moving_avg = (float*) pvPortMalloc(NUM_LEDS * sizeof(float));
  for (int i = 0; i < NUM_LEDS; i++) {
    fft_moving_avg[i] = 0;
  }
  float* moving_avg_weight = (float*) pvPortMalloc(sizeof(float));
  ...
  // place LED data variables in queues
  if(xQueueSend(
    Q_fft_moving_avg, &fft_moving_avg, portMAX_DELAY) != pdPASS) {
    Serial.println("Failed to allocate Q_fft_moving_avg");
  }
  if(xQueueSend(
    Q_moving_avg_weight, &moving_avg_weight, portMAX_DELAY) != pdPASS) {
    Serial.println("Failed to allocate moving_avg_weight");
  }
  if(xQueueSend(
  ...
  // launch choose colorscale task
  bool created = false;
  created = xTaskCreate(Task_ChooseColors,
              "Choose Colorscale Task",
              STACK_DEPTH_COLORS,
              NULL,
              PRIORITY_COLORS,
              NULL);
  if (!created) {
    Serial.println("Setup: Failed to create Choose Colorscale Task");
  } 
...
void Task_SetLEDs(void *pvParameters) {
  ...
  // Wait for use FFT output and moving average data
  xQueueReceive(Q_fft_output, &fft_output, portMAX_DELAY);
  xQueueReceive(Q_fft_moving_avg, &fft_moving_avg, portMAX_DELAY);
  ...
  *brightness_val = brightness_setting;
  ...
  // Wait for use of moving avg weight ratio data
  xQueueReceive(Q_moving_avg_weight, &weight_val, portMAX_DELAY);

  // set moving avg weight ratio from analog input
  weight_setting = (float) analogRead(PIN_MA_RATIO) / MAX_ADC_VAL;
  *weight_val = weight_setting;

  // release use of moving avg weight ratio data
  if(xQueueSend(
    Q_moving_avg_weight, &weight_val, portMAX_DELAY) != pdPASS) {
    Serial.println("LED Task: Failed to send Q_moving_avg_weight");
  }
  ...
  // set LEDS
  LED_colorscale_set32(fft_moving_avg,
                       fft_output,
                       led_settings,
                       color_palette_ptr,
                       NUM_LEDS,
  ...
  // release use of FFT data arrays
  if(xQueueSend(
    Q_fft_output, &fft_output, portMAX_DELAY) != pdPASS) {
  ...
  if(xQueueSend(
    Q_fft_moving_avg, &fft_moving_avg, portMAX_DELAY) != pdPASS) {
    Serial.println("LED Task: Failed to send Q_fft_moving_avg.");
  }
...
void Task_ChooseColors(void* pvParameters) {
  CRGB* color_palette_ptr;
  int scales[NUM_COLOR_SCALES][NUM_COLORS] = COLOR_SCALES_ARRAY;
  int current_scale_index = 0;

  // initialize button input pin
  pinMode(PIN_COLOR_SCALE, INPUT);

  // initialize color palette values
  xQueueReceive(Q_palette, &color_palette_ptr, portMAX_DELAY); 
  for (int i = 0; i < NUM_COLORS; i++) {
    color_palette_ptr[i] = CRGB(scales[0][i]);
  }
  if(xQueueSend(
    Q_palette, &color_palette_ptr, portMAX_DELAY) != pdPASS) {
    Serial.println("CHOOSE COLORS: Failed to send Q_palette");
  }
  for (;;) {
    // change color scale if button pressed
    if (digitalRead(PIN_COLOR_SCALE) == HIGH) {
      // alternate through color scale array indices
      current_scale_index = (current_scale_index + 1) % NUM_COLOR_SCALES;
      // wait for use of palette array
      xQueueReceive(Q_palette, &color_palette_ptr, portMAX_DELAY);

      // update LED palette colors
      for (int i = 0; i < NUM_COLORS; i++) {
        color_palette_ptr[i] = CRGB(scales[current_scale_index][i]);
      }
      if(xQueueSend(
        Q_palette, &color_palette_ptr, portMAX_DELAY) != pdPASS) {
        Serial.println("CHOOSE COLORS: Failed to send Q_palette");
      }
      // delay task to debounce button
      vTaskDelay((float) DEBOUNCE_MILLIS / portTICK_PERIOD_MS);
    }

    // Get the high water mark for debugging
    if (__DEBUG_MEM_STAT__) {
      UBaseType_t uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
      Serial.print("CHOOSE COLORS: highwatermark (32-bit words): ");
      Serial.println(uxHighWaterMark);
      xHeapStats heapstats;
      vPortGetHeapStats(&heapstats);
      Serial.print("    Largest free heap block -- "); 
    ...
    // delay task to reduce CPU usage
    vTaskDelay((float) DEFAULT_ROUTINE_TASK_DELAY / portTICK_PERIOD_MS);
  }
}
