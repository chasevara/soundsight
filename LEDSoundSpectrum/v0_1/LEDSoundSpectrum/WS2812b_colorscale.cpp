...
void LED_colorscale_set32(float* vals_avg, 
                          float* vals_update, 
                          CRGB* leds,
  ...
  for (int i = 1; i < num_leds; i++) { 
    ...
    // cast subtraction operands to ints to avoid hanging on 
    // floating point subtractionerror
    vals_avg[i] 
      = vals_avg[i] + weight_moving_avg 
        * ((int32_t) val_update - (int32_t) vals_avg[i]);
